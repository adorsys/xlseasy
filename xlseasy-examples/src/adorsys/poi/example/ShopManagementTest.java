...
import adorsys.poi.example.Client;
import adorsys.poi.example.Product;
import adorsys.poi.example.ShopManagement;
import adorsys.poi.example.MyExcelFunction;
...
public class ShopManagementTest extends MyExcelFunction {
	...
	public void createExcel() {
		...
		try {
			...
			// the following line allow to create a sheet and add sheet's name with special characters such as [], ? # + %$¤"
			...
			Product product2 = new Product("Samsung Galaxy Tab 2 10.1", "Tablet", 340.0);
			Product product3 = new Product("Seagate USB", "USB Stick - 8GB", 6.0);
			...
			for (int i = 2; i < managerListArray.length + 2; i++) {
				...
				cellClient.setCellValue(managerListArray[i - 2].getClient().getId());
				
				cellClient = rowClient.createCell(2);
				setMyCellAlignment(book, cellClient, defaultAlignment);
				cellClient.setCellValue(managerListArray[i - 2].getClient().getName());
				
				cellClient = rowClient.createCell(3);
				setMyCellAlignment(book, cellClient, defaultAlignment);
				cellClient.setCellValue(managerListArray[i - 2].getClient().getStreet());
				
				cellClient = rowClient.createCell(4);
				setMyCellAlignment(book, cellClient, defaultAlignment);
				cellClient.setCellValue(managerListArray[i - 2].getClient().getZipcode());
				
				cellClient = rowClient.createCell(5);
				setMyCellAlignment(book, cellClient, defaultAlignment);
				cellClient.setCellValue(managerListArray[i - 2].getClient().getCity());
				
				cellClient = rowClient.createCell(6);
				setMyCellAlignment(book, cellClient, defaultAlignment);
				cellClient.setCellValue(managerListArray[i - 2].getClient().getCountry());
			}
			...
			for (int i = 2; i < managerListArray.length + 2; i++) {
				...
				cellProduct.setCellValue(managerListArray[i - 2].getProduct().getName());

				cellProduct = rowProduct.createCell(2);
				setMyCellAlignment(book, cellProduct, defaultAlignment);
				cellProduct.setCellValue(managerListArray[i - 2].getProduct().getDesc());
				
				cellProduct = rowProduct.createCell(3);
				setMyCellAlignment(book, cellProduct, defaultAlignment);
				cellProduct.setCellValue(managerListArray[i - 2].getProduct().getPrice());				
			}
			...
			String[] headerTitle = { "Client's ID", "Client", "Delivery address", "Article",
